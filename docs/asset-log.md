# 素材制作日志

## 2026-05-09

### 已完成

- 免费默认主图：`assets/girls/girl01/free.png`
- 解锁奖励主图：`assets/girls/girl01/unlocked.png`
- 扩展收藏/落地页图库已接入 9 张：
  - `hero-stadium-host.jpg`
  - `hero-goalkeeper-free.jpg`
  - `hero-fan-girl.jpg`
  - `hero-black-gold-vip.jpg`
  - `keeper-idle.jpg`
  - `keeper-left-dive.jpg`
  - `keeper-right-dive.jpg`
  - `keeper-victory.jpg`
  - `unlock-trophy-gold.jpg`

### 接入位置

GitHub Pages 实际使用根目录，因此以下三处需要同步：

1. 根目录：`index.html`, `game.js`, `style.css`, `assets/`
2. 源码目录：`penalty-girls/`
3. 部署备份目录：`penalty-girls-deploy/`

### 下一批应补齐

- 更完整的守门动作：中路扑救、失败沮丧、进球后惊讶。
- 更多解锁皮肤：红色球衣、白金球衣、雨夜球场、训练场、经理助理、女主持。
- 落地页 A/B：绿茵清爽版、黑金奖励版、蓝绿科技 UI 版。
